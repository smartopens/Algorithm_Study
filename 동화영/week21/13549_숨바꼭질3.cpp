...
// N지점에서 K지점까지 최소 시간으로 도달하기
// 이동 방법은 3가지 - +1칸 이동, -1칸 이동, *2칸으로 순간이동
// PQ를 활용하기
// 1. 현재 지점에서 3가지 방법 중 가능한 케이스를 PQ에 넣는다.
// 2. PQ에서 시간 순으로 오름차순 정렬을 하여 짧은 시간부터 pop하여 탐색한다.
// 3. pop한 지점에서 다시 1을 반복한다.
